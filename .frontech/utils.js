const [fs, route, colors, translateTokens, svgSpreact, figma, shelljs, SVGFixer] = [
  require("fs"),
  require('path'),
  require("colors"),
  require("./tokens"),
      ...
      if (file && fixer) {
        ...
        exec(`node ${_cli} ${files} --config ${webfonts}`, { async: true, silent: false }, async (code) => {
          ...
          if (success) {
            ...
            const creation = createFile(route.resolve(process.cwd(), path, 'library/scss/base'), '_icons.scss', _data, true);
...
function RGBAToHex(rgba) {
  const _replacePad = rgba.split('').filter(item => !['#'].includes(item)).join('');
