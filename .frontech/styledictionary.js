const [fs, utils, route, StyleDictionary] = [
  require('fs'),
  require("./utils"),
  require("path"),
  require("style-dictionary")
];
const { createFile, config, dataFilesScss, RGBAToHex, translateReferenceToCustomProperty } = utils;
...
const createCustomProperties = (tokens) => {
  return tokens.reduce((tokens, prop) => {
    ...
    if (!isString) {
      ...
      customVar += `--${name}:${translateReferenceToCustomProperty(value, name)};\n`;
    }
...
const styleDictionary = (file, path) => {
  ...
  try {
    ...
    _styleDictionary.registerFormat({
      ...
      formatter: ({ dictionary: { allTokens, getReferences, usesReference } }) => {
